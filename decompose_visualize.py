...
def activation_extract(G, D, E, G_ema, fixed_x, fixed_y, z_, y_,
                    state_dict, config, experiment_name, device, normal_eval, eval_vc, return_mask, block='3-2'):
    ...
    with torch.no_grad():
        if config['parallel']:
            ...
            output = which_G(fixed_z, which_G.shared(fixed_y), int(1 / config['sparse_decay_rate']) + 100, \
                            return_inter_activation=True, device=device, normal_eval=normal_eval, eval_vc=eval_vc, return_mask=return_mask)
            print("output.shape", len(output))
            fixed_Gz = output[0]
            print("fixed_Gz.shape", fixed_Gz.shape)
            intermediate = output[1]
            print(f"intermediate {intermediate[block].shape}")
            

    # # print("check3 -----------------------------")
    save_dir = '%s/%s/iter_%s' % (config['evals_root'], experiment_name, state_dict['itr'])
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    
    # save the figure
    image_origin_filename = '%s/origin_iter_%s.jpg' % (save_dir,
                                                        state_dict['itr'])
    image_filename = '%s/test_iter_%s.jpg' % (save_dir,
                                            state_dict['itr'],)
    print("######### save_image_path #####: ", image_filename)
    ...
    # save each channel
    image_index = 2
    channel_image_filename = '%s/test_iter_%s_img_%s_block_%s_channels_.jpg' % (save_dir,
                                                    state_dict['itr'],
                                                    str(image_index), 
                                                    block)
    activation = intermediate[block].cpu()
    # sort activation by activation magnitude
    activation_sort = torch.argsort(activation[image_index].std((1,2)), descending=True)
    activation = activation[image_index, activation_sort].unsqueeze(1).repeat(1, 3, 1, 1)
    
    torchvision.utils.save_image(activation.float(), channel_image_filename,
                                 nrow=int(activation.shape[0] ** 0.5), normalize=True)

    # activation 10%?
    sparsity = torch.where(activation == 0)[0]
