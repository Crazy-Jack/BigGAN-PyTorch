""" BigGAN: The Authorized Unofficial PyTorch release
    Sparisify the activation and see the output images
"""
...
def activation_extract(G, D, E, G_ema, fixed_x, fixed_y, z_, y_,
                    state_dict, config, experiment_name, sparsity=0.1, device='cuda:0'):
    ...
    with torch.no_grad():
        if config['parallel']:
            ...
            fixed_Gz = nn.parallel.data_parallel(
                which_G, (fixed_z, which_G.shared(fixed_y), False, sparsity, device))
        else:
            # print("fixed_x: ", fixed_x.shape)
            ...
            fixed_Gz = which_G(fixed_z, which_G.shared(fixed_y), return_inter_activation=False, sparsity=sparsity, device=device)
    print("check3 -----------------------------")
    if not os.path.isdir('%s/%s' % (config['samples_root'], experiment_name)):
        os.mkdir('%s/%s' % (config['samples_root'], experiment_name))
    image_filename = '%s/%s/sparsity_layer0_%f_test_iter_%s.jpg' % (config['samples_root'],
                                                    experiment_name, sparsity,
                                                    state_dict['itr'])
    image_origin_filename = '%s/%s/sparsity_layer0_%f_origin_iter_%s.jpg' % (config['samples_root'],
                                                    experiment_name, sparsity,
                                                    state_dict['itr'])
...
def run(config, target_sparsity_list):
    ...
    for i, (x, y) in enumerate(pbar):
        ...
        for sparsity in target_sparsity_list:
            print("On sparsity {} ------- ".format(sparsity))
...
def main():
    ...
    print(config)
    target_sparsity_list = [0.0, 0.01, 0.05, 0.10, 0.25, 0.5, 0.7]
    # target_sparsity_list = [1]
    run(config, target_sparsity_list)
