...
def prepare_parser():
    ...
    parser.add_argument(
        '--lambda_vae_recon', type=float, default=0.1,
        help='vae reconstruction weight '
        '(default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--select_index', type=str, default="-1",
    ...
    parser.add_argument(
        '--conv_select_kernel_size', type=int, default=5,
        help="conv selection kernel size"
    )
    
    parser.add_argument(
        '--vc_dict_size', type=int, default=150,
        help="size of vc cadidate in conv selection module"
    )
    
    parser.add_argument(
        '--sparse_vc_interaction_num', type=int, default=10,
...
class Distribution(torch.Tensor):
    ...
    def sample_(self):
        if self.dist_type == 'normal':
            ...
            raise NotImplementedError("dist_type {} is not implemneted for utils.Distribution.sample_".format(self.dist_type))
...
def prepare_z_y(G_batch_size, dim_z, nclasses, device='cuda',
                fp16=False, z_var=1.0, vae=False):
    if not vae:
